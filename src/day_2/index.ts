...
const answer1 = sum(
  input
    ...
    .map((round) => {
      ...
      return (
        myResponseToScore[myResponse] +
        outcomeToScore[roundToOutcome[round as Round]]
      );
    })
);
...
const answer2 = sum(
  input
    ...
    .map((strategy) => {
      ...
      return (
        myResponseToScore[strategyToMyResponse[strategy as Strategy]] +
        outcomeToScore[outcome]
      );
    })
);
